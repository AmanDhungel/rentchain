...
const TenantStatusBadge: React.FC<{ status: TenantStatus }> = ({ status }) => {
  let classes =
    "inline-flex items-center px-2 py-1.5 text-xs font-medium rounded-sm";
  let dotClasses = "w-2 h-2 rounded-full mr-1 bg-white";
  const text = status;
...
const OverdueStatus: React.FC<{
  ...
  if (amount === null) {
    return <span className="text-green-500 font-medium">Current</span>;
  }
  return (
...
const SubleaseStatusBadge: React.FC<{ status: SubleaseStatus }> = ({
  ...
  const classes = "px-2 py-1.5 text-xs font-medium rounded-[1px]";
  let colorClass = "";

  switch (status) {
    case "Enabled":
      colorClass = "bg-orange-500 text-white rounded-[4px]";
      break;
    case "Disabled":
      colorClass = "bg-slate-700 text-white rounded-[4px] text-gray-600";
      break;
  }
      ...
      icon = (
        <>
          <div className="mt-2.5">
            ...
            />
            <Slash className="relative w-3 h-3 rotate-90 -top-3.5 left-0.5 text-red-500 z-50" />
          </div>
